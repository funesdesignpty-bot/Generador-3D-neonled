// =============================================================
// GENERADOR DE LETREROS NEON FLEX PRO PLUS - V5.4 (SLIDERS FIX)
...
module motor_svg(dist, es_int = false) {
    mirror([(Espejo == "si" ? 1 : 0), 0, 0]) {
        if (es_int) { offset(delta = dist) import(archivo_svg, center = true); }
        else { offset(r = dist) import(archivo_svg, center = true); }
    }
}
    ...
    if (act == "si") {
        translate([x, y, 0]) {
            if (Tipo_de_Anclaje == "Keyhole") {
                hull() { cylinder(d=Diam_Anclaje+6, h=H_Oreja); translate([0, L_Keyhole-5, 0]) cylinder(d=Diam_Anclaje+6, h=H_Oreja); }
            } else { cylinder(d=Diam_Anclaje+6, h=H_Oreja); }
        }
    }
}
...
difference() {
    union() {
        ...
        if (Ver != "solo_letras") {
            color("GhostWhite", 0.4) union() {
                linear_extrude(height = Alt_Base) motor_svg(Margen_Base, es_int = false);
        ...
        if (Ver != "solo_base") {
            color("DeepSkyBlue") translate([0, 0, (Ver == "solo_letras" ? 0 : Alt_Base)]) {
                difference() {
                    linear_extrude(height = H_Total_Neon) motor_svg((Ancho_Canal/2) + Grosor_Pared_Canal, es_int = false);
                    translate([0, 0, Fondo_Canal]) linear_extrude(height = H_Total_Neon + 1) motor_svg(Ancho_Canal/2, es_int = true);
                    // Limpieza externa
                    translate([0, 0, -1]) linear_extrude(height = H_Total_Neon + 2) difference() {
                        motor_svg((Ancho_Canal/2) + Grosor_Pared_Canal + 300, es_int = false);
                        motor_svg((Ancho_Canal/2) + Grosor_Pared_Canal, es_int = false);
                    }
                }
            }
        }
    ...
    for(a=anclajes) if(a[0]=="si") translate([a[1], a[2], -5]) {
        if (Tipo_de_Anclaje == "Keyhole") {
            ...
            hull() { cylinder(d=Diam_Anclaje, h=H_Oreja+Alt_Base+20); translate([0, L_Keyhole-5, 0]) cylinder(d=Diam_Anclaje, h=H_Oreja+Alt_Base+20); }
        } else { cylinder(d=Diam_Anclaje, h=H_Oreja+Alt_Base+20); }
    }
