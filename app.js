// =============================================================
// MOTOR DE LECTURA DE NEON FLEX PRO PLUS - V7.0 (CON WEB WORKER)
// Funes Design 360 - David, Chiriquí
// =============================================================

const NOMBRE_ARCHIVO_SCAD = "letras.scad";

let worker = null;         // El trabajador en segundo plano que corre el motor 3D
let motorListo = false;    // Si el worker ya terminó de cargar OpenSCAD
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // 2. Construimos los sliders/selectores según lo que diga el .scad
        construirInterfaz(scadOriginal);

        // 3. Preparamos el visor 3D (vacío por ahora, hasta que generes un modelo)
        initViewer3D();

        // 4. Arrancamos el motor 3D en un hilo aparte (Web Worker), para que
        //    la pantalla nunca se sienta congelada mientras calcula
        worker = new Worker("worker.js?v=2", { type: "module" });

        worker.onmessage = (evento) => {
            const msg = evento.data;
            if (msg.tipo === "log") {
                agregarLog(msg.texto);
            } else if (msg.tipo === "listo") {
                motorListo = true;
                setEstado("Listo. Sube tu archivo SVG y presiona “Generar Modelo 3D”.");
            } else if (msg.tipo === "resultado") {
                ultimoSTL = msg.datos;
                mostrarSTL(msg.datos);
                ...
                agregarLog("❌ Falló la generación: " + msg.mensaje);
                document.getElementById('btn-render').disabled = false;
            }
        };

        worker.onerror = (err) => {
            setEstado("❌ Error interno del motor. Revisa el registro de abajo.");
            agregarLog("❌ " + (err.message || "Error desconocido en worker.js"));
            document.getElementById('btn-render').disabled = false;
        };
    } catch (err) {
        console.error(err);
...
document.getElementById('svg-file').addEventListener('change', (evento) => {
    ...
    lector.onload = () => {
        try {
            ...
            // Detectamos la codificación real mirando los primeros bytes (BOM)
            ...
            // Como guardaremos el archivo internamente en UTF-8, corregimos la
            // declaración XML para que coincida con lo que realmente se va a escribir
...
// -------------------------------------------------------------
function compilar() {
    if (!motorListo) { setEstado("El motor todavía no está listo, espera un momento."); return; }
    if (!svgTexto) { setEstado("⚠️ Primero sube tu archivo SVG (arriba a la izquierda)."); return; }

    setEstado("Generando modelo 3D en segundo plano... (puede tardar varios segundos, la pantalla seguirá respondiendo con normalidad)");
    document.getElementById('btn-render').disabled = true;

    const scadFinal = reconstruirScad(scadOriginal);

    // Le mandamos el trabajo al worker; la respuesta llega en worker.onmessage
    worker.postMessage({ scadTexto: scadFinal, svgTexto: svgTexto });
}
